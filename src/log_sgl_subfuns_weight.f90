...
CONTAINS
  ...
  SUBROUTINE log_update_step_weight(bsg, startix, endix, b, lama, t_for_sg, pfg, pfl1, lam1ma,&
       x, y, isDifZero, nobs, r, gamg, maxDif, nvars, lb, ub, weight)
      IMPLICIT NONE
      ...
      s = MATMUL( weight*y - 1/(1.0D0+exp(-r))  , x(:, startix:endix))/nobs
      ...
      RETURN
   END SUBROUTINE log_update_step_weight
   ...
   SUBROUTINE log_sp_update_step(bsg, startix, endix, b, lama, t_for_sg, pfg, pfl1,&
      ...
      oldb = b(startix:endix)
      ! print *, oldb
      CALL spatx(x,xidx, xcptr, nobs, nvars, nnz, weight*y - 1.0D0/(1.0D0+exp(-r)), s, startix, endix)
...
END MODULE log_sgl_subfuns_weight
