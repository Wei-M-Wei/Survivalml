...
SUBROUTINE log_spmat_four (bn,bs,ix,iy,gam,nobs,nvars,x,xidx,xcptr,nnz,y,pf,pfl1,&
     ...
     activeGroup,nbeta,alam,npass,jerr,alsparse,lb,ub,weight)
  ! --------------------------------------------------
  USE log_sgl_subfuns
  USE sgl_subfuns
  USE spmatmul
  ...
  ! PRINT *, alf
  vl = 0.0D0
  CALL spatx(x, xidx, xcptr, nobs, nvars, nnz, weight*y- 1.0D0/(1.0D0+exp(-r)), vl, 1, nvars)
  vl = vl / nobs
  al0 = 0.0D0
  DO g = 1, bn ! For each group...
     ALLOCATE(u(bs(g)))
  ...
  DO WHILE (l < nlam)
     ...
     DO
        ...
        DO
           ...
           DO g = 1, bn
              ...
              CALL log_sp_update_step(bs(g), startix, endix, b, lama, t_for_s(g),&
                   ...
                   r, gam(g), maxDif, nvars, lb(g), ub(g), weight)
              IF (activeGroupIndex(g) == 0 .AND. isDifZero == 1) THEN
                 ni = ni+1
                 IF (ni > pmax) EXIT
                 activeGroupIndex(g) = ni
        ...
        CALL sp_strong_kkt_check(is_in_E_set, violation, bn, ix, iy, pf,pfl1,&
             ...
             weight*y- 1.0D0/(1.0D0+exp(-r)), nobs, nvars, vl)
        IF (violation == 1) CYCLE
        ! Need to compute vl/ga for the ones that aren't already updated,
        ! before log_kkt_check
        ! CALL rchkusr()
        DO g = 1, bn
           IF (is_in_S_set(g) == 0) THEN
              ...
              ALLOCATE(s(bs(g)))
              s = 0.0D0
              CALL spatx(x, xidx, xcptr, nobs, nvars, nnz, weight*y- 1.0D0/(1.0D0+exp(-r)),&
                   s, startix, endix)
