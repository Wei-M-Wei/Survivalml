# Script: application 1 for github (s = 6) another group structure 2 (grouped by correlation matrix).R
# Corporate Survival Analysis with Machine Learning Methods
# Purpose: Reproduce Application 1 for s = 6 using groups defined by the correlation matrix.
#============================================================
...
lag_use_year = 6
quarter = 4
lags = lag_use_year * quarter - 1 # Information about the latest lag is unavailable.
jmax <- lags



# Read the data and inspect basic information
# To replicate the results, import the subset extracted by the algorithm in the paper.
data_financial = read.csv2("period_final_new6.csv")  # Read 'period_final_new6.csv'; change the file name if needed.
...
for (g in c(1,2,3,4,5,6)) {
  ...
  for (t in c( 8, 8.5, 9)) {
    ...
    result_para = foreach(k = 1:it, .errorhandling= 'remove', .packages = packages_to_export, .export = export_env) %dopar% {
      ...
      # --- Step 2: Hierarchical clustering based on correlation ---
      dist_mat <- as.dist(1 - cor_mat)        # Convert correlations to distances
      hc <- hclust(dist_mat, method = "average")  # or "ward.D2"

      X_in <- as.matrix(cbind(rep(1, nrow(X_train)), Xdw))
      gindex <- cutree(hc, k = g)
    ...
    # Save the table
    table_name1 <- paste0(g, "cross_another_G_MIDAS_AUC", s, "_AUC_cvm", t, ".csv")
    table_name2 <- paste0(g, "cross_another_G_MIDAS_AUC", s, "_AUC", t, ".csv")
    table_name3 <- paste0(g, "cross_another_G_MIDAS_bootstrap", s, "_AUC_cvm", t, ".csv")
    table_name4 <- paste0(g, "cross_another_G_MIDAS_bootstrap", s, "_AUC", t, ".csv")
    table_name5 <- paste0(g, "cross_another_G_MIDAS", s, "_cof", t, ".csv")
    table_name6 <- paste0(g, "cross_another_G_MIDAS", s, "_cof_AUC", t, ".csv")
    table_name7 <- paste0(g, "cross_another_G_MIDAS_LASSO", s, "_cof", t, ".csv")
    table_name8 <- paste0(g, "cross_another_G_MIDAS_LASSO", s, "_cof_AUC", t, ".csv")
