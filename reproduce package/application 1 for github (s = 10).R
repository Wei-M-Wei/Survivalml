...
t = 13
######################################################################### Part 1
# prediction horizons t = 13, 13.5, 14 years, Estimator(5) in this paper:SGL-MIDAS, LASSO-MIDAS and LASSO-UMIDAS
...
################################################################# Part 2
# prediction horizons t = 13, 13.5, 14 years, Comparison model without censoring
for (t in c( 13, 13.5, 14)) {
  # iteration number
  it = 10
  result_para = foreach(k = 1:it, .errorhandling= 'remove', .packages = packages_to_export, .export = export_env) %dopar% {
