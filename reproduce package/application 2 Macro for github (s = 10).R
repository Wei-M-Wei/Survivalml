# Script: application 2 Macro for github (s = 10).R
# Corporate Survival Analysis with Machine Learning Methods
# Purpose: Reproduce the macro-augmented Application 2 results for s = 10.
#============================================================


#============================================================
# Reproduce empirical Application 2 for s = 10 years and t = 13, 13.5, and 14 years
#============================================================
...
for (t in c(13, 13.5, 14)) {
  ...
  ##############################################################
  X_test_macro = X_test[, 1: (p_macro) ]
  X_test_fin = X_test[, (p_macro+1):dim(X_test)[2] ]
  ##################################################
  ...
  gindex_macro <- NULL
  Xdw_macro = NULL
  for (z in seq(dim(X_train_macro)[2]/jmax)){
  ...
  gindex_fin <- NULL
  Xdw_fin = NULL
  for (z in seq(dim(X_train_fin)[2]/jmax)){
    z_idx <- (1 + (z - 1) * jmax) : (z * jmax)
    Xdw_fin <- cbind(Xdw_fin, X_train_fin[,z_idx] %*% w_fin)
  ...
  for (z in seq(dim(X_test_macro)[2]/jmax)){
    z_idx <- (1 + (z - 1)*jmax) : (z * jmax)
    X_t_macro <- cbind(X_t_macro, X_test_macro[,z_idx]%*%w_macro)
  }
  X_t_fin = NULL
  for (z in seq(dim(X_test_fin)[2]/jmax)){
    z_idx <- (1 + (z - 1)*jmax) : (z * jmax)
    X_t_fin <- cbind(X_t_fin, X_test_fin[,z_idx]%*%w_fin)
  }
  ...
  # Save the table
  table_name1 <- paste0("application2_Macro_MIDAS_AUC", s, "_AUC_cvm", t, ".csv")
  table_name2 <- paste0("application2_Macro_MIDAS_AUC", s, "_AUC", t, ".csv")
  table_name3 <- paste0("application2_Macro_MIDAS_bootstrap", s, "_AUC_cvm", t, ".csv")
  table_name4 <- paste0("application2_Macro_MIDAS_bootstrap", s, "_AUC", t, ".csv")
  table_name5 <- paste0("application2_Macro_MIDAS", s, "_cof", t, ".csv")
  table_name6 <- paste0("application2_Macro_MIDAS", s, "_cof_AUC", t, ".csv")
  table_name7 <- paste0("application2_Macro_MIDAS_LASSO", s, "_cof", t, ".csv")
  table_name8 <- paste0("application2_Macro_MIDAS_LASSO", s, "_cof_AUC", t, ".csv")
  table_name9 <- paste0("application2_Macro_LASSO", s, "_cof", t, ".csv")
  table_name10 <- paste0("application2_Macro_LASSO", s, "_cof_AUC", t, ".csv")
