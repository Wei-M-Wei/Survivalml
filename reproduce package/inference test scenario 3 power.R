# Script: inference test scenario 3 power.R
# Corporate Survival Analysis with Machine Learning Methods
# Purpose: Evaluate inference power under simulation scenario 3.
#============================================================


#============================================================
# Run the scenario-3 inference power simulation
#============================================================
...
generateData_AR <-
  function(s, n, numhv, numtrue, degree, jmax, parameters = NA, censor_strength){
    ...
    for ( tt in 1:(jmax)){
      if (tt == 1){
        Xdd[,tt,(1:p)] <- rmvt(TN, sigma = corr_matrix, df = 2) # Draw the initial innovations
      }
      else{
        Xdd[,tt,(1:p)] <- rho * Xdd[,tt-1,(1:p)] + rmvt(TN, sigma = corr_matrix*(1 - rho^2), df = 2 ) # Update the autoregressive covariates
      }
    }
...
# Extract the inference metrics for scenario 3
...
# Calculate the censoring rate for a simulated scenario-3 dataset
...
print(pa[i, ])
table_name <- paste0("MIDASAUC_scenario3_inference_power", censor, 1200, s,  ".csv")
