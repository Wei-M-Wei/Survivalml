...
#============================================================
# Reproduce simulation, scenario 4 Table 7
#============================================================
...
generateData_AR <-
  function(s, n, numhv, numtrue, degree, jmax, parameters = NA, censor_strength){
    ...
    for ( tt in 1:(jmax)){
      if (tt == 1){
        Xdd[,tt,(1:p)] <- rmvt(TN, sigma = corr_matrix, df = 2) #scenario 4
      }
      else{
        Xdd[,tt,(1:p)] <- rho * Xdd[,tt-1,(1:p)] + rmvt(TN, sigma = corr_matrix*(1 - rho^2), df = 2 ) #scenario 3
      }
    }
    ...
    # grab the prediction metric for scenario 4
    ...
    # calculate the censoring rate of a simulation dataset (scenario 4)
