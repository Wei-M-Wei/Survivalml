...
names_pic = c("Accounts receivable turnover ratio", "Current assets turnover ratio"
...
ggplot(melted_data, aes(Time, Variables, fill = factor(Value))) +  # Use factor for discrete values
  geom_tile(color = "black") +  # Add border to tiles
  scale_fill_manual(values = c("0" = "white", "1" = "red")) +  # Custom colors
  labs(title = "Selected financial covariates when s = 6 years.",
...
ggplot(melted_data, aes(Time, Variables, fill = Value)) +  # Use factor for discrete values
  ...
  scale_fill_gradient(low = "white", high = "red") +  # Custom colors
  labs(title = "Sparsity pattern for selected types, s = 6 years.",
       x = "Prediction Horizon",
       y = "Variables",
       fill = "Value") +
  theme_minimal()
...
ggplot(melted_data, aes(Time, Variables, fill = factor(Value))) +  # Use factor for discrete values
  geom_tile(color = "black") +  # Add border to tiles
  scale_fill_manual(values = c("0" = "white", "1" = "red")) +  # Custom colors
  labs(title = "Sparsity pattern for selected covariates.",
       x = "Prediction Horizon",
       y = "Variables") +
  theme_minimal() +
  theme(legend.position = "none")
