...
for (censor in censor_strength){
  ...
  for (t in t_quantile){
    ...
    pa[i, ] = c(res$censoring_proportions, res$balance,
                ...
                res$AUC_true_N_VAR, res$AUC_LASSO_N_VAR, res$AUC_MIDAS_LASSO_N_VAR, res$AUC_MIDAS_N_VAR,
                'w1',
  ...
  table_name <- paste0("MIDASAUC_scenario1", censor, 800, s,  ".csv")
...
for (censor in censor_strength){
  ...
  table_name <- paste0("MIDASAUC_scenario1", censor, 1200, s,  ".csv")
  write.table(pa, table_name, sep = ";", row.names = FALSE, quote = FALSE)
}
