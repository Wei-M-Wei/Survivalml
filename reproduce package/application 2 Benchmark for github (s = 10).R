...
for (t in c(13, 13.5, 14)) {
  ...
  fit_bench = survival_sparsegl(X_train, y_train, group = seq(dim(X_train)[2]), lambda = c(0), weight = w_train, asparse = 1, intercept_zero = intercept_zero, standardize = TRUE, eps = 1e-8, maxit = 60000)
