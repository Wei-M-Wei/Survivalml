...
sgl_logit <- function(
  ...
  if (!is.sparse) {
    ...
    fit <- dotCall64::.C64(
      ...
      # Read only
      alsparse = as.double(asparse), lb = lower_bnd, ub = upper_bnd, weight = as.double(weight),
      INTENT = c(rep("r", 14), rep("rw", 8), rep("w", 8), rep("r", 4)))
  }
  # output
...
cv.survival_sparsegl <- function(
    ...
    nfolds = 10, foldid = NULL, offset = NULL, weight, intercept_zero, AUC = FALSE, maxit = 20000,
    ...) {
  ...
  cvstuff <- cverror(survival_sparsegl.object, outlist, lambda, x, y, foldid,
                     pred.loss, weight = weight, AUC = AUC)
  cvm <- cvstuff$cvm
  cvsd <- cvstuff$cvsd
...
cverror <- function(fullfit, outlist, lambda, x, y, foldid, pred.loss, weight, AUC, ...) {
  UseMethod("cverror")
}

#' @export
cverror.logitspgl <- function(
    fullfit, outlist, lambda, x, y, foldid, weight = weight, AUC = AUC,
    pred.loss = c("default", "mse", "deviance", "mae", "misclass", 'censor', 'AUC_censor'),
    ...) {
  ...
  AUC_censor = rep(0, dim(predmat)[2])
  if (AUC == TRUE){
    for ( i in seq(dim(predmat)[2])){
      if (any(is.na(predmat[,i]))){
        AUC_censor[i] = 0
      } else{
        AUC_censor[i] = survival::survivalROC(Stime=train_dataset$time, status= train_dataset$status, marker = predmat[,i], predict.time = t, span = 0.25*length(train_dataset)^(-0.20) )$AUC
      }
    }
  } else{
  ...
  cvraw <- switch(
    ...
    censor = censor,
    AUC_censor = rbind(AUC_censor, AUC_censor),
